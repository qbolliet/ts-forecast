"""Explicit imputation plan produced by ``HighFrequencyImputer``.
...
# Sentinelle de repli : simple constante, pas une hiérarchie (foyer canonique).
INTERPOLATE_FALLBACK = 'interpolate_fallback'


# Alias de typage internes au module (redéfinis localement pour éviter tout
# couplage de module inutile)
...
class ImputationPlan:
    """Immutable ordered container of :class:`ImputationStep`.
...
def append_step(plan: ImputationPlan, step: ImputationStep) -> ImputationPlan:
    ...
    The PHASE 5 build of ``HighFrequencyImputer`` grows its plan through
